...
<body>
	<?php 
	if(empty($_GET['action']) || empty($_GET['ma'])) header("location:../quan_ly_admin/");
	$ma = $_GET['ma']; // lấy mã của admin trên thanh địa chỉ
	$action = $_GET['action']; // lấy hành động trên thanh địa chỉ 
	require '../common/connect.php';
	$sql_check_ma = "select ma from admin where ma = $ma";
			...
			<div class="insert_update kh">
				...
				<form action="process.php" method="post">
					<?php if(isset($_GET['error'])) { ?>
						<h4 style="color: red; opacity: 0.8; margin-top: 20px"><?php echo $_GET['error'] ?></h4>
					<?php } ?>
					<input type="hidden" name="ma" value="<?php echo $ma ;?>">
					<input type="hidden" name="action" value="<?php echo $action ;?>">
					<h1 style="text-align: center; padding-top: 5px">Sửa thông tin của admin</h1>
					<span class="chu_do">*</span>
					<span>(bắt buộc điền)</span>
					<table width="100%">
						<tr>
							<td>
								<span class="in_dam">Tên Admin</span> <br>
							...
							<td>
								...
								<tr>
									<td>
										...
										<input type="text" name="cmnd" placeholder="Nhập số chứng minh" value=" <?php echo $each['cmnd'] ?>">
									</td>
								</tr>
								<tr>
									<td>
